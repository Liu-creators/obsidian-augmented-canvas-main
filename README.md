...
选择多个节点，输入连线指令（如"按时间顺序连线"、"找出因果关系"），AI 会分析节点关系并自动创建合适的连接线。
...
**使用方法**: 选择 2+ 节点 → 右键菜单 → Smart Connect → 输入连线指令

#### 🗂️ Smart Grouping (智能分组)
**多选节点，AI 智能分类！**

选择散落的节点,输入分组指令（如"按技术栈分类"、"按优先级分组"），AI 会自动创建分组并归类节点。
...
**使用方法**: 选择 2+ 节点 → 右键菜单 → Smart Grouping → 输入分组指令

#### 🎨 Type-Based Coloring (类型着色)
**语义化节点类型，自动颜色标记！**

AI 会根据内容语义自动选择节点类型和颜色：
- 🟠 **概念 (Concept)** - 核心概念和关键术语
- 🔵 **步骤 (Step)** - 流程步骤和技术实现
- 🟢 **资源 (Resource)** - 参考资料和引用
- 🔴 **警告 (Warning)** - 风险和注意事项
- 🟣 **洞察 (Insight)** - 总结和结论
- 🟡 **问题 (Question)** - 待办事项和思考点
- ⚪ **默认 (Default)** - 普通文本

#### 📐 Grid Coordinate System (网格坐标)
**基于网格的智能布局系统！**

使用行列坐标 (row/col) 替代绝对像素坐标，AI 可以更智能地排列节点位置：
- 相对于源节点的逻辑定位
- 支持负坐标（向上/向左）
- 可配置节点大小和间距
- 适配不同显示分辨率
...
全新 XML 格式替代 Markdown 分隔符，支持更复杂的结构：
- `<node>`: 节点定义（类型、标题、坐标、内容）
- `<group>`: 分组定义（嵌套节点）
- `<edge>`: 连接定义（方向、标签）
- `<member>`: 成员引用（Smart Grouping）

**向后兼容**: 自动检测格式，完全兼容原 Markdown 格式。
...
#### 3. 生成包含多个节点的 Group ✨ 新功能

**一键生成结构化内容！** AI 可以一次性创建包含多个相关节点的 Group，而不是单个节点。

**主要特性**：
- 🎯 **智能多节点生成**：AI 根据上下文生成 3-6 个相关节点，每个节点包含不同的视角和内容
- 📐 **智能布局**：自动根据节点数量选择最佳布局（水平、网格等）
- 🎨 **可配置**：自定义 Group 颜色、节点间距、内边距等参数
- 📝 **Markdown 格式**：支持使用 `###` 标题或 `---` 分隔符来标记多个节点
